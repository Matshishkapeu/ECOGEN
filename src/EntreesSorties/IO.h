...
public:
  ...
  //Definition de template pour ecriture au format binaire
  template <typename T>
  static std::ostream& write(std::ostream &fluxSortie, T &valeur)
  {
    //Swap Little <-> Big endian eventuel
    IO::endswap(&valeur);
    return fluxSortie.write(reinterpret_cast<char*>(&valeur), sizeof(T));
  };

  // //Definition de template pour lecture au format binaire
  // template <typename T>
  // static std::istream& read(std::istream &fluxEntree, T& valeur)
  // {
  //   return fluxEntree.read(reinterpret_cast<char*>(&valeur), sizeof(T));
  //   //Swap Little <-> Big endian eventuel
  //   IO::endswap(&valeur);
  ...
  template <typename T>
  static void ajouteAlaChaine(char *chaine, int &taille, T &valeur)
  {
    char *conversionChaine = reinterpret_cast<char*>(&valeur);
    for (int octet = 0; octet < sizeof(valeur); octet++)
    { chaine[taille++] = conversionChaine[octet]; }  
  }

  //Definition de template pour ecriture d un nombre au format binaire base64
  template <typename T>
  static std::ostream& writeb64(std::ostream &fluxSortie, T &valeur)
  {
    //Swap Little <-> Big endian eventuel ne marche pas ??
    //IO::endswap(&valeur);
    //Encodage Base64
    char* chaine = reinterpret_cast<char*>(&valeur);
    int tailleChaine = sizeof(valeur);
    return IO::writeb64Chaine(fluxSortie, chaine, tailleChaine);
  };

  // //ATTENTION !!!!!!!!!!!Lecture non Fonctionnelle !!!!!!!!!!!!!!
  // //Definition de template pour lecture au format binaire base64
  // template <typename T>
  // static std::istream& readb64(std::istream &fluxEntree, T& valeur)
  // {
  //   return fluxEntree.read(reinterpret_cast<char*>(&valeur), sizeof(T));
  //   //Swap Little <-> Big endian eventuel
  //   IO::endswap(&valeur);
  ...
  static void copieFichier(std::string fichier, std::string dossierSource, std::string dossierDestination);
...
/*
----------------Mode d emploi binaire-----------------
exemple avec un entier code sur 4 octets

int valeur = 256 487 423
Sa representation binaire avec les bits de poids fort a gauche est :
...
char *tableau = reinterpret_cast<char *>(valeur)
...
la representation binaire en paquet de 6 bits. Dans notre cas, cela devient:
...
On associe ensuite a chaque code binaire sur 6 bits un caractere simple parmis les 64 suivants:
...
qui aura la representation binaire final (Pas sur pour le = si c'est sa representation ASCII ou pas)
00000011 00110100 00100110 00101111    00111111 00110000 00111101 00111101

------------------------------------------------------

zone de tests inversion b64
CwsLCwsLCws=    //chaine binaire 64 d'entiers sur 8 bits
000010 110000 101100 001011 000010 110000 101100 001011 000010 110000 101100  //Traduction binaire
