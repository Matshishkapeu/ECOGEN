...
CondLimInj::CondLimInj(){}

//****************************************************************************

CondLimInj::CondLimInj(int numPhysique, XMLElement *element, vector<Phase*> vecPhases, int &nombreTransports, std::vector<std::string> nomTransports, string nomFichier) :
  CondLim(numPhysique)
{
  ...
  //Lecture du debit massique surfacique (kg/s/m²)
  XMLElement *sousElement(element->FirstChildElement("donneesInjection"));
  if (sousElement == NULL) throw ErreurXMLElement("donneesInjection", nomFichier, __FILE__, __LINE__);
  //Recuperation des attributs
  //--------------------------
  XMLError erreur;
  erreur = sousElement->QueryDoubleAttribute("m0", &m_m0);
  if (erreur != XML_NO_ERROR) throw ErreurXMLAttribut("m0", nomFichier, __FILE__, __LINE__);
...
CondLimInj::CondLimInj(double m0, double *ak0, double *rhok0, double *pk0, const int nombrePhases)
{
  m_nombrePhase = nombrePhases;
...
CondLimInj::CondLimInj(const CondLimInj &Source, const int lvl) : CondLim(Source)
{
...
void CondLimInj::creeLimite(BordDeMaille **face)
{
  *face = new CondLimInj(*(this));
}

//****************************************************************************

void CondLimInj::resolRiemannLimite(Cellule &cellGauche, const int & nombrePhases, const double & dxGauche, double & dtMax)
{
  m_mod->resolRiemannInj(cellGauche, nombrePhases, dxGauche, dtMax, m_m0, m_ak0, m_rhok0, m_pk0);
}

//****************************************************************************

void CondLimInj::resolRiemannTransportLimite(Cellule &cellGauche, const int & nombreTransports) const
{
	m_mod->resolRiemannTransportInj(cellGauche, nombreTransports, m_valeurTransport);
}

//****************************************************************************

void CondLimInj::afficheInfos()
{
...
void CondLimInj::creerBordEnfant()
{
  m_bordsEnfants.push_back(new CondLimInj(*this, m_lvl + 1));
}
