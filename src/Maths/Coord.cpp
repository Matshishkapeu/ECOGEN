//  
//       ,---.     ,--,    .---.     ,--,    ,---.    .-. .-. 
//       | .-'   .' .')   / .-. )  .' .'     | .-'    |  \| | 
//       | `-.   |  |(_)  | | |(_) |  |  __  | `-.    |   | | 
//       | .-'   \  \     | | | |  \  \ ( _) | .-'    | |\  | 
//       |  `--.  \  `-.  \ `-' /   \  `-) ) |  `--.  | | |)| 
//       /( __.'   \____\  )---'    )\____/  /( __.'  /(  (_) 
//      (__)              (_)      (__)     (__)     (__)     
//
//  This file is part of ECOGEN.
//
//  ECOGEN is the legal property of its developers, whose names 
//  are listed in the copyright file included with this source 
//  distribution.
//
//  ECOGEN is free software: you can redistribute it and/or modify
//  it under the terms of the GNU General Public License as published 
//  by the Free Software Foundation, either version 3 of the License, 
//  or (at your option) any later version.
//  
//  ECOGEN is distributed in the hope that it will be useful,
//  but WITHOUT ANY WARRANTY; without even the implied warranty of
//  MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
//  GNU General Public License for more details.
//  
//  You should have received a copy of the GNU General Public License
//  along with ECOGEN (file LICENSE).  
//  If not, see <http://www.gnu.org/licenses/>.

#include <algorithm>
#include <cmath>
#include "Coord.h"
#include <iostream>

using namespace std;

//*********************************************************************

Coord::Coord() : m_x(0.), m_y(0.), m_z(0.){}

//*********************************************************************

Coord::Coord(const double &x, const double &y, const double&z) :
m_x(x), m_y(y), m_z(z)
{}

//*********************************************************************

Coord::~Coord(){}

//*********************************************************************

void Coord::setXYZ(const double &x, const double & y, const double &z)
{
  m_x = x;
  m_y = y;
  m_z = z;
}

//*********************************************************************

void Coord::setX(const double &x){m_x = x;}

//*********************************************************************

void Coord::setY(const double &y){m_y = y;}

//*********************************************************************

void Coord::setZ(const double &z){m_z = z;}

//*********************************************************************

double Coord::getX() const
{
  return m_x;
}

//*********************************************************************

double Coord::getY() const
{
  return m_y;
}

//*********************************************************************

double Coord::getZ() const
{
  return m_z;
}

//*********************************************************************

double Coord::norme() const
{
  return sqrt(m_x*m_x+m_y*m_y+m_z*m_z);
}

//*********************************************************************

double Coord::normeCarre() const
{
  return m_x*m_x + m_y*m_y + m_z*m_z;
}

//*********************************************************************

double Coord::scalaire(const Coord &a) const
{
  return m_x*a.m_x + m_y*a.m_y + m_z*a.m_z;
}

//*********************************************************************

Coord Coord::vectoriel(const Coord &a) const
{
  Coord vec;
  vec.m_x = m_y*a.m_z - m_z*a.m_y;
  vec.m_y = m_z*a.m_x - m_x*a.m_z;
  vec.m_z = m_x*a.m_y - m_y*a.m_x;
  return vec;
}

//*********************************************************************

void Coord::projection(const Coord &normale, const Coord &tangente, const Coord &binormale)
{
  Coord vecteurProjete;
  vecteurProjete.m_x = this->scalaire(normale);
  vecteurProjete.m_y = this->scalaire(tangente);
  vecteurProjete.m_z = this->scalaire(binormale);
  *this = vecteurProjete;
}

//*********************************************************************

void Coord::projectionRepereAbsolu(const Coord &normale, const Coord &tangente, const Coord &binormale)
{
  Coord vecteurProjete;
  vecteurProjete.m_x = normale.m_x*m_x + tangente.m_x*m_y + binormale.m_x*m_z;
  vecteurProjete.m_y = normale.m_y*m_x + tangente.m_y*m_y + binormale.m_y*m_z;
  vecteurProjete.m_z = normale.m_z*m_x + tangente.m_z*m_y + binormale.m_z*m_z;
  *this = vecteurProjete;
}

//*********************************************************************

void Coord::creeVecteur(const Coord &a, const Coord &b)
{
  m_x = b.m_x - a.m_x;
  m_y = b.m_y - a.m_y;
  m_z = b.m_z - a.m_z;
}

//*********************************************************************

double Coord::produitScalaire(const Coord &v1, const Coord &v2)
{
  return v1.m_x*v2.m_x + v1.m_y*v2.m_y + v1.m_z*v2.m_z;
}

//*********************************************************************

Coord Coord::produitVectoriel(const Coord &v1, const Coord &v2)
{
  Coord vec;
  vec.m_x = v1.m_y*v2.m_z - v1.m_z*v2.m_y;
  vec.m_y = v1.m_z*v2.m_x - v1.m_x*v2.m_z;
  vec.m_z = v1.m_x*v2.m_y - v1.m_y*v2.m_x;
  return vec;
}

//*********************************************************************

void Coord::changeSigne()
{
  m_x = -m_x;
  m_y = -m_y;
  m_z = -m_z;
}

//*********************************************************************

void Coord::normalise()
{
  *this = *this / this->norme();
}

//*********************************************************************

double Coord::determinant(const Coord &v1, const Coord &v2, const Coord &v3)
{
  double det(0.); 
  det =  (v1.getX()*v2.getY()*v3.getZ()) + (v1.getY()*v2.getZ()*v3.getX()) + (v1.getZ()*v2.getX()*v3.getY());
  det -= (v3.getX()*v2.getY()*v1.getZ()) + (v3.getY()*v2.getZ()*v1.getX()) + (v3.getZ()*v2.getX()*v1.getY());
  return det;
}

//*********************************************************************

double Coord::cos(const Coord &v1, const Coord &v2)
{
  double cos(0.);
  double nv1(v1.norme()), nv2(v2.norme());
  if (nv1 > 1e-6 && nv2 > 1e-6) { cos = v1.scalaire(v2) / (nv1*nv2); }
  return cos;
}

//*********************************************************************

Coord Coord::sin(const Coord &v1, const Coord &v2)
{
  Coord sin;
  double nv1(v1.norme()), nv2(v2.norme());
  if (nv1 > 1e-6 && nv2 > 1e-6) { sin = v1.vectoriel(v2) / (nv1*nv2); }
  return sin;
}

//*********************************************************************

void Coord::afficheInfos() const
{
  cout << " X = " << m_x << " Y = " << m_y << " Z = " << m_z << endl;
}

//*******************SURCHARGES OPERATEURS*****************************

Coord& Coord::operator=(const double &scalaire)
{
  m_x = scalaire;
  m_y = scalaire;
  m_z = scalaire;
  return *this;
}

//*********************************************************************

Coord& Coord::operator*= (const double &scalaire)
{
  m_x *= scalaire;
  m_y *= scalaire;
  m_z *= scalaire;
  return *this;
}

//*********************************************************************

Coord& Coord::operator/= (const double &scalaire)
{
  m_x /= scalaire;
  m_y /= scalaire;
  m_z /= scalaire;
  return *this;
}

//*********************************************************************

Coord Coord::operator* (const double &scalaire)
{
  Coord copie(*this);
  copie *= scalaire;
  return copie;
}

//*********************************************************************

Coord Coord::operator/ (const double &scalaire)
{
  Coord copie(*this);
  copie /= scalaire;
  return copie;
}

//*********************************************************************


Coord& Coord::operator+= (const Coord& a)
{
  m_x += a.m_x;
  m_y += a.m_y;
  m_z += a.m_z;
  return *this;
}

//*********************************************************************

Coord& Coord::operator-= (const Coord &a)
{
  m_x -= a.m_x;
  m_y -= a.m_y;
  m_z -= a.m_z;
  return *this;
}

//*********************************************************************
//Surcharge operateur externe a la classe car prends deux arguments

Coord operator* (const double &scalaire, const Coord &a)
{
  Coord copie(a);
  copie *= scalaire;
  return copie;
}

Coord operator+ (const Coord &a, const Coord &b)
{
  Coord copie(a);
  copie += b;
  return copie;
}

Coord operator- (const Coord &a, const Coord &b)
{
  Coord copie(a);
  copie -= b;
  return copie;
}
